...
public class QuerydslNoOffsetNumberOptions<T, N extends Number & Comparable<?>> extends QuerydslNoOffsetOptions<T> {
    ...
    public QuerydslNoOffsetNumberOptions(@NotNull NumberPath<N> field, @NotNull Expression expression) {
    ...
    public void initKeys(JPAQuery<T> query, int page) {
        if(page == 0) {
            ...
            System.out.println("_________________________ first key = " + currentId + "  last key = " + lastId);
        }
    }
    ...
    public void resetCurrentId(T item) {
        ...
        System.out.println("________________ Current Select Key = " + currentId);
    }
}
